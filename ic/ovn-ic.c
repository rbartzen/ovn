                    ...
                    struct ic_router_info *ic_lr)
{
    ...
    for (int i = 0; i < ic_lr->n_isb_pbs; i++) {
        ...
        ts_lrp = get_lrp_by_lrp_name(ctx, lrp_name);
        if (ts_lrp) {
            ts_route_table = smap_get_def(&ts_lrp->options, "route_table", "");
        } else {
            ts_route_table = "";
        }
                  ...
                  struct shash *routes_ad_by_ts)
{
    ...
    for (int i = 0; i < ic_lr->n_isb_pbs; i++) {
        ...
        ts_lrp = get_lrp_by_lrp_name(ctx, lrp_name);
        if (ts_lrp) {
            route_table = smap_get_def(&ts_lrp->options, "route_table", "");
        } else {
            route_table = "";
        }
        build_ts_routes_to_adv(ctx, ic_lr, routes_ad, &ts_port_addrs,
                               nb_global, route_table, ts_lrp);
